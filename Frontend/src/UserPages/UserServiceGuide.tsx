      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="space-y-4">
          {filteredServices.map((service) => (
            <div
              key={service.id}
              className="group bg-white border border-gray-200 rounded-xl p-5 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer flex items-center justify-between"
            >
              <div className="flex items-center gap-4 flex-1">
                <div className={`p-2.5 rounded-lg ${service.iconBg} ${service.iconColor}`}>
                  <service.icon size={24} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
                  <div>
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${service.category === 'ID Application' ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'
                      }`}>
                      {service.category}
                    </span>
        ...
        <div className="bg-green-50 border border-green-100 rounded-xl p-6">
          ...
          <div className="flex gap-3 mt-4">
            <button className="px-5 py-2 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-700">
              Contact Us
            </button>
            <button className="px-5 py-2 bg-white border border-green-200 text-green-600 text-sm font-bold rounded-lg hover:bg-green-50">
              Appointment
            </button>
          </div>
        </div>
