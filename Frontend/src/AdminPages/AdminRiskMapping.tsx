            ...
            <main>
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 bg-white  border border-gray-100 shadow-sm p-6">
                        ...
                        <div className="aspect-video bg-blue-50  relative overflow-hidden flex items-center justify-center border border-blue-100">
                            <div className="text-center p-8 max-w-md bg-white/80 backdrop-blur-sm  border border-white shadow-xl z-10">
                            ...
                            <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-red-200/40  blur-2xl" />
                            <div className="absolute bottom-1/3 right-1/4 w-40 h-40 bg-red-200/40  blur-3xl" />
                        ...
                        <div className="bg-white  border border-gray-100 shadow-sm p-6">
                            ...
                            <div className="space-y-3">
                                {residentLocations.filter(r => r.isHighRisk).slice(0, 2).map(r => (
                                    <div key={r.id} className="p-4 bg-red-50  border border-red-100 relative group cursor-pointer hover:bg-red-100 transition">
                                        <p className="font-bold text-gray-900 text-sm">{r.name}</p>
                                        <p className="text-[11px] text-gray-500 mb-2">{r.id}</p>
                                        <div className="flex flex-wrap gap-1">
                                            <span className="px-2 py-0.5 bg-white text-red-600 text-[10px] font-bold  border border-red-200">Flood Risk</span>
                                            <span className="px-2 py-0.5 bg-white text-gray-600 text-[10px] font-bold  border border-gray-200 truncate">{r.vulnerability}</span>
                                        </div>
                        ...
                        {residentLocations.map((resident) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    {resident.isHighRisk && (
                                        <span className="px-2 py-1 bg-red-600 text-white text-[10px] font-black uppercase  tracking-tighter shadow-sm">
                                            High Risk
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100/50">
                                    <span className="inline-block px-3 py-1 bg-white border border-gray-200  text-[11px] font-bold text-gray-700 shadow-sm group-hover:border-gray-300">
...
const LegendItem = ({ color, label, isDot = false }: { color: string; label: string; isDot?: boolean }) => (
    <div className="flex items-center gap-2">
        <div className={`${color} ${isDot ? 'w-3 h-3  shadow-sm' : 'w-6 h-4  shadow-sm'}`} />
