...
const ResidentModal = ({ isOpen, onClose, initialData }: ResidentModalProps) => {
    ...
    const getSafeUrl = (filePath: string | undefined | null) => {
        if (!filePath || filePath === 'null' || filePath === '') return "";
    ...
    const [formData, setFormData] = useState<any>({
        ...
        photo_2x2: null as File | string | null,
        proof_of_residency: null as File | string | null,
    });
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData) {
                ...
                // Helper to extract file paths from the attachments array
                const findFile = (type: string) => {
                    return initialData.attachments?.find(a => a.file_type === type)?.file_path || null;
                };
        ...
        switch (step) {
            case 1:
                return (
                    ...
                    formData.birthplace && formData.nationality && formData.contact_number &&
                    formData.email
                );
            case 2:
                return (
                    formData.house_no && formData.street && formData.barangay &&
            ...
            case 3:
                ...
                return basicValid;
            case 4:
                // If editing, files are optional unless the user removes existing ones
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (initialData) {
                const identifier = initialData.system_id;
                await axios.put(`${API_BASE_URL}/api/user/${identifier}`, data, config);
            ...
            const errorMsg = err.response?.data?.message || 'Submission failed. Please check your connection.';
                    ...
                    {step === 3 && (
                        <div className="space-y-4 animate-in fade-in duration-300">
                            ...
                            <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 space-y-3">
                                ...
                                <Checkbox
                                    label="Resident is a PWD (Person with Disability)"
                                    name="is_pwd"
                            ...
                            <div className="space-y-4">
                                ...
                                <div className={`p-4 border-2 border-dashed rounded-xl transition-colors ${formData.photo_2x2 ? 'border-green-200 bg-green-50/30' : 'border-slate-200 hover:border-blue-400'}`}>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">
                                        Upload 2x2 Photo <span className="text-red-500">*</span>
                                    </label>
                                    <div className="flex items-center gap-4">
                                        <div className="h-24 w-24 bg-slate-100 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 overflow-hidden shadow-inner">
                                            {formData.photo_2x2 ? (
                                                <img
                                                    src={getImagePreview(formData.photo_2x2)!}
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <input
                                                type="file"
                                                accept="image/*"
                                                onChange={(e) => handleFileChange(e, 'photo_2x2')}
                                                className="text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 cursor-pointer"
                                            />
                                            {typeof formData.photo_2x2 === 'string' && (
                                                <span className="text-[10px] text-blue-600 font-medium italic">
                                                    Currently on file: {formData.photo_2x2.split('/').pop()}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className={`p-4 border-2 border-dashed rounded-xl transition-colors ${formData.proof_of_residency ? 'border-green-200 bg-green-50/30' : 'border-slate-200 hover:border-blue-400'}`}>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">
                                        Upload Proof of Residency <span className="text-red-500">*</span>
                                    </label>
                                    <div className="flex flex-col gap-2">
                                        <input
                                            type="file"
                                            onChange={(e) => handleFileChange(e, 'proof_of_residency')}
                                            className="text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer"
                                        />
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-slate-500 bg-white p-2.5 rounded-lg border border-slate-100 shadow-sm">
                                                ...
                                                <span className="truncate flex-1 font-medium">
                                                    {formData.proof_of_residency instanceof File
                                                        ? formData.proof_of_residency.name
                                                        : `Stored: ${formData.proof_of_residency.split('/').pop()}`}
                                                </span>

                                                <div className="flex items-center gap-3 ml-2 border-l pl-3 border-slate-200">
                                                    {typeof formData.proof_of_residency === 'string' && (
                                                        <a
                                                            href={getSafeUrl(formData.proof_of_residency)}
                                                            target="_blank"
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => setFormData({ ...formData, proof_of_residency: null })}
                    ...
                    <div className="flex gap-3">
                        {step < 4 ? (
                            <button
                                ...
                                className={`px-8 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg active:scale-95 ${isStepValid() ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200' : 'bg-slate-300 text-slate-500 shadow-none cursor-not-allowed'}`}
                            >
                            ...
                            <button
                                ...
                                className={`px-10 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all active:scale-95 ${isStepValid() && !loading ? 'bg-slate-900 hover:bg-black text-white' : 'bg-slate-300 text-slate-500 cursor-not-allowed'}`}
                            >
...
const Input = ({ label, required, ...props }: any) => (
    <div className="space-y-1.5">
        ...
        <input {...props} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all placeholder:text-slate-400" />
    </div>
);
...
const Select = ({ label, options, required, ...props }: any) => (
    <div className="space-y-1.5">
        ...
        <select {...props} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all appearance-none cursor-pointer">
