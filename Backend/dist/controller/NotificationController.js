...
export class NotificationController {
    ...
    async sendSupportRequest(req, res) {
        try {
            ...
            const [residentRows] = await db.query("SELECT CONCAT(firstname, ' ', lastname) as full_name FROM users WHERE id = ?", [user_id]);
            if (residentRows.length === 0) {
                return res.status(404).json({ error: "Sender not found in database." });
            }
            const residentName = residentRows[0].full_name;
            await db.query(`INSERT INTO support_tickets (user_id, message, channel) VALUES (?, ?, ?)`, [user_id, message, channel]);
            const [admins] = await db.query("SELECT email, contact_number, firstname FROM users WHERE role = 1");
            if (admins.length === 0) {
