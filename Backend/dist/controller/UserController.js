...
class UserController {
    ...
    AddNewUser = async (req, res) => {
        ...
        const { firstname, middlename, lastname, suffix, email, contact_number, gender, birthday, birthplace, nationality, civil_status, blood_type, house_no, street, barangay, ownership_type, years_of_residency, residence_status, occupation, monthly_income, education, school, household_number, type, id_expiry_date, tcic_id, disability, is_flood_prone, is_registered_voter, emergencyContact } = req.body;
        ...
        try {
            ...
            if (type === 'Both')
                prefix = 'SC-PWD';
            else if (['Senior Citizen', 'SC'].includes(type))
                prefix = 'SC';
            else if (type === 'PWD')
                prefix = 'PWD';
            ...
            if (parsedEmergencyContact && parsedEmergencyContact.name) {
                await connection.execute(`INSERT INTO emergency_contacts (user_id, name, relationship, contact) VALUES (?, ?, ?, ?)`, [
                    newIdFromDB,
            ...
            if (files) {
                ...
                for (const fieldName of attachmentTypes) {
                    if (files[fieldName]) {
                        ...
                        await connection.execute(`INSERT INTO user_attachments (user_id, file_type, file_data, file_name, mime_type) 
                 VALUES (?, ?, ?, ?, ?)`, [
                            newIdFromDB,
                            fieldName,
    ...
    updateUser = async (req, res) => {
        ...
        WHERE id = ?`, [
            ...
            if (parsedEmergencyContact && parsedEmergencyContact.name) {
                await connection.execute(`INSERT INTO emergency_contacts (user_id, name, relationship, contact) 
             VALUES (?, ?, ?, ?) 
            ...
            if (files) {
                ...
                for (const fileType of attachmentTypes) {
                    if (files[fileType]) {
                        ...
                        await connection.execute(`DELETE FROM user_attachments WHERE user_id = ? AND file_type = ?`, [userId, fileType]);
    ...
    getAllResidentLocations = async (req, res) => {
        try {
            const [rows] = await pool.execute(`SELECT id, system_id, firstname, lastname, house_no, street, barangay, disability, coordinates, is_flood_prone 
             FROM users 
             WHERE role = 2`);
