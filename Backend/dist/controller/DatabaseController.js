    ...
    async getAivenStatus(req, res) {
        ...
        try {
            ...
            try {
                ...
                try {
                    const [tables] = await connection.execute(`SELECT 
                            TABLE_NAME as table_name, 
                            COALESCE(TABLE_ROWS, 0) AS table_rows, 
                            ROUND(COALESCE(DATA_LENGTH + INDEX_LENGTH, 0) / 1024 / 1024, 2) AS size_mb 
                         FROM information_schema.tables 
                         WHERE table_schema = ? 
                         ORDER BY (DATA_LENGTH + INDEX_LENGTH) DESC`, [process.env.DB_NAME || 'defaultdb']);
                    tableStats = tables;
                }
                catch (dbErr) {
                ...
                try {
                    const [slowQueries] = await connection.execute(`SELECT 
                            query, 
