...
class AppointmentController {
    getAttachment = async (req, res) => {
        const { id } = req.params;
        try {
            const [rows] = await pool.execute('SELECT attachment FROM appointments WHERE id = ?', [id]);
    ...
    updateStatus = async (req, res) => {
        ...
        try {
            ...
            const [appointmentData] = await pool.execute(`SELECT a.service_type, a.appointment_date, a.appointment_time, u.email, u.firstname, u.contact_number 
                 FROM appointments a
    ...
    createAppointment = async (req, res) => {
        ...
        try {
            ...
            const [user] = await pool.execute(`SELECT email, firstname FROM users WHERE id = ?`, [user_id]);
            if (user.length > 0) {
                notificationService.sendBroadcastNotification({
    ...
    deleteAppointment = async (req, res) => {
        ...
        try {
            const [rows] = await pool.execute('SELECT user_id FROM appointments WHERE id = ?', [id]);
            if (rows.length === 0) {
                return res.status(404).json({ message: "Appointment not found." });
            }
