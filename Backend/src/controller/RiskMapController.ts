...
class RiskMapController {
    public async getAllResidentLocations(req: Request, res: Response): Promise<Response> {
        try {
    ...
    // GET all flood zones
    public async getFloodZones(req: Request, res: Response): Promise<Response> {
        try {
            const [rows] = await pool.execute<RowDataPacket[]>(
                'SELECT id, lat, lng, radius FROM flood_zones'
            );
    ...
    public async createFloodZone(req: Request, res: Response): Promise<Response> {
        const { lat, lng, radius } = req.body;
        try {
            const [result] = await pool.execute<ResultSetHeader>(
                'INSERT INTO flood_zones (lat, lng, radius) VALUES (?, ?, ?)',
                [lat, lng, radius]
            );
    ...
    public async updateFloodZone(req: Request, res: Response): Promise<Response> {
        const { id } = req.params;
        const { radius } = req.body;
        try {
            await pool.execute(
                'UPDATE flood_zones SET radius = ? WHERE id = ?',
                [radius, id]
            );
    ...
    // DELETE a flood zone
    public async deleteFloodZone(req: Request, res: Response): Promise<Response> {
        const { id } = req.params;
        try {
